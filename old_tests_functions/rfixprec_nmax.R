...
fixprec <- function(n, J, N, S, total, kappa = NULL, active = NULL, details = FALSE) {
  ...
  a.vec <- as.matrix(tapply(N * S, J, sum) / rho)
  c.vec <- tapply(N * S^2, J, sum) / rho2 # - b (b = 0 if M = N)
  ...
  if (details) {
    kkt <- check_kkt(x, J0, N0, S0, total, kappa, n0, active, s_d, details = TRUE)
...
#' check_kkt(x, J, N, S, total, kappa, n)
#' check_kkt(x, J, N, S, total, kappa, n, details = TRUE)
#'
check_kkt <- function(n_opt, J, N, S, total, kappa, n, active = NULL, s = NULL, tol = 10^-9, details = FALSE) {
  strata_ind <- seq_along(n_opt)

  Td_notscaled <- tapply(N^2 / n_opt * S^2 - N * S^2, domain_indicators(J), sum)
  ...
  repeat {
    ...
    for (d in I_nrec) {
      ...
      if (non_empty(violated)) {
        if (d >= 2 && non_empty(W_nrec_active[1:(d - 1)])) { # wyczysc temp active z poprzednich domen
